/* **************************************************************************
 File Name	:	EN772_SENS_IMX462.c
 Description	:	EN772_801 - IMX462 device driver
 Designer		:	Park Chan
...
#if(0)
#define AE_GAIN_UNIT_MAX		(238)	    																	// Sensor gain step by Unit  (IMX462:71.4dB)
#define AE_GAIN_UNIT_EA			(1)																				// Sensor gain Unit EA
#define AE_GAIN_UNIT_OVR		(29)//(81)																		// Sensor gain extended step (HCG)
...
#else
#define AE_GAIN_UNIT_MAX		(238*3)	    																	// Sensor gain step by Unit  (IMX462:71.4dB)
#define AE_GAIN_UNIT_EA			(1)																				// Sensor gain Unit EA
#define AE_GAIN_UNIT_OVR		(28*3)//(81)																		// Sensor gain extended step (HCG)
#define AE_GAIN_UNIT_OVR2		(0)	    																		// ISP gain extended step
...
//#define AE_LWDR_2P_RHS1			((gbInModeRes==MD_2M_FPS_125_20)?(19):(61))		// 1250 line Test
...
// initial
	...
	const WORD gwTblSens0[][5] = {	// normal, 2p DOL
								//ADDR	 Nor 2M30P	DOL 2P	Nor 2M60P	DOL 2P
								/*ID2*/
								{0x3005, 	0x01, 	0x00,	0x01,		0x00}, 	// ADBIT
								{0x3007, 	0x00, 	0x00,	0x00,		0x00}, 	// WINMODE (1080p)
								{0x3009, 	0x02, 	0x01,	0x01,		0x00}, 	// FRSEL *
								{0x300a, 	0x00,	0x00,	0x00,		0x00},	// BLKLEVEL (recommend : 0xf0 at 12b mode)
								{0x300b, 	0x00, 	0x00,	0x00,		0x00}, 	// "
								{0x300c, 	0x00, 	0x11,	0x00,		0x11},	// WDSEL/WDMODE (normal : 0x0, DOL2p : 0x11)
//								{0x300e, 	0xf0, 	0xf0,	0xf0,		0xf0},	// fixed
								{0x300f, 	0x00, 	0x00,	0x00,		0x00},	// fixed		
								{0x3010, 	0x21, 	0x61,	0x21,		0x61}, 	// FPGC : each frame gain of DOL 0x61, common gain of DOL 0x21 (normal, datasheet default is wrong)
								{0x3011, 	0x02, 	0x02,	0x02,		0x02}, 	// (IMX462) fixed at DATASHEET 0.3
								{0x3012, 	0x64, 	0x64,	0x64,		0x64},	// fixed
								{0x3016, 	0x09, 	0x09,	0x09,		0x09},	// fixed
								
//								{0x3018, 	0x65, 	0x65,	0x65,		0x65}, 	// VMAX
//								{0x3019, 	0x04, 	0x04,	0x04,		0x04}, 	// "
//								{0x301a, 	0x00, 	0x00,	0x00,		0x00}, 	// "
//								
//								{0x301c, 	0x30, 	0x98,	0x98,		0x4c}, 	// HMAX *
//								{0x301d, 	0x11, 	0x08,	0x08,		0x04}, 	// "

								{0x3020, 	0x02,	0x02,	0x02, 		0x02}, 	// SHS1
								{0x3021, 	0x00,	0x00,	0x00, 		0x00}, 	// "
								{0x3022, 	0x00,	0x00,	0x00, 		0x00},	// "

								{0x3024, 	0x00,	0x87,	0x00, 		0x43}, 	// SHS2
								{0x3025, 	0x00,	0x08,	0x00, 		0x08}, 	// "
								{0x3026, 	0x00,	0x00,	0x00, 		0x00},	// "

								{0x3030, 	0x00,	0x07,	0x00, 		0x09}, 	// RHS1
								{0x3031, 	0x00,	0x00,	0x00, 		0x00}, 	// "
								{0x3032, 	0x00,	0x00,	0x00, 		0x00},	// "
								
								{0x3045, 	0x01, 	0x05,	0x01,		0x05},	// DOLSCDEN, DOLSYDINFOEN, HINFOEN
								{0x3046, 	0x01,	0x00,	0x01, 		0x00}, 	// OPORTSEL, ODBIT (normal, DOL)						//MIPI
//								{0x304b, 	0x0a, 	0x0a,	0x0a,		0x0a}, 	// XVSOUTSEL, XHSOUTSEL (V,H SYNC ON)
//								{0x3048, 	0x00,	0x10,	0x00, 		0x10}, 	// XVSLNG				(VSYNC Low pulse width)
//								{0x3049, 	0x0a,	0x1a,	0x0a, 		0x1a}, 	// XHSLNG				(HSYNC Low pulse width)
								{0x305c, 	0x18, 	0x18,	0x18,		0x18}, 	// INCKSEL1
								{0x305d,	0x03,	0x03,	0x03,		0x03},	// INCKSEL2 
								{0x305e, 	0x20, 	0x20,	0x20,		0x20}, 	// INCKSEL3
								{0x305f, 	0x01, 	0x01,	0x01,		0x01}, 	// INCKSEL4
								{0x3070, 	0x02, 	0x02,	0x02,		0x02}, 	// fixed
								{0x3071, 	0x11, 	0x11,	0x11,		0x11}, 	// fixed								
								{0x309b, 	0x10, 	0x10,	0x10,		0x10}, 	// fixed
								{0x309c, 	0x21, 	0x21,	0x21,		0x21}, 	// fixed							
//								{0x309e, 	0x4a, 	0x4a,	0x4a,		0x4a}, 	// (IMX462) fixed
//								{0x309f, 	0x4a, 	0x4a,	0x4a,		0x4a}, 	// (IMX462) fixed
								{0x30a2,	0x02,	0x02,	0x02,		0x02},	// (IMX462) fixed
								{0x30a6,	0x20,	0x20,	0x20,		0x20},	// (IMX462) fixed
								{0x30a8,	0x20,	0x20,	0x20,		0x20},	// (IMX462) fixed
								{0x30aa,	0x20,	0x20,	0x20,		0x20},	// (IMX462) fixed
								{0x30ac,	0x20,	0x20,	0x20,		0x20},	// (IMX462) fixed
								{0x30b0,	0x43,	0x43,	0x43,		0x43},	// (IMX462) fixed
//					            {0x30b1,	0x00,	0x00,	0x00,		0x01},	// (IMX462) fixed
//					            {0x30c0,	0x10,	0x14,	0x10,		0x14},	// (IMX462) fixed
					            {0x30f0, 	0xf0,	0x64,	0xf0, 		0x64},	// S1 gain off
					                                                	    
								/*ID3*/                             	    
								{0x3106, 	0x00, 	0x15,	0x00,		0x15}, 	// XVS,HVS subsampling when DOL (2p master : 0x15)
								{0x3119, 	0x9e, 	0x9e,	0x9e,		0x9e}, 	// 
								{0x311c, 	0x1e, 	0x1e,	0x1e,		0x1e}, 	// 
								{0x311e,	0x08,	0x08,	0x08,		0x08},	// fixed
								{0x3128,	0x05,	0x05,	0x05,		0x05},	// fixed
								{0x3129,	0x00,	0x1d,	0x00,		0x1d},	// ADBIT1
								{0x313d,	0x83,	0x83,	0x83,		0x83},	// WINWV
								{0x3150,	0x03,	0x03,	0x03,		0x03},	// 
//								{0x3153,	0x40,	0x40,	0x40,		0x40},	// DU_LTCFLG
								{0x315e, 	0x1a, 	0x1a,	0x1a,		0x1a}, 	// INCKSEL5
								{0x3164, 	0x1a, 	0x1a,	0x1a,		0x1a}, 	// INCKSEL6
								{0x317c, 	0x00, 	0x12,	0x00,		0x12}, 	// ADBIT2
								{0x317e,	0x00,	0x00,	0x00,		0x00},	//
								{0x31a0,	0xfc,	0xb4,	0xfc,		0xb4},	//
								{0x31a1,	0x00,	0x02,	0x00,		0x02},	//
								{0x31ec, 	0x0e, 	0x37,	0x0e,		0x37},	// (IMX462) ADBIT3
	                                                                	    
								/*ID4*/                             	    
								{0x3257,	0x03,	0x03,	0x03,		0x03},	// fixed
								{0x3264,	0x1a,	0x1a,	0x1a,		0x1a},	// INCKSEL6
								{0x3265,	0xb0,	0xb0,	0xb0,		0xb0},	// fixed
								{0x3266,	0x02,	0x02,	0x02,		0x02},	// fixed
								{0x326b,	0x10,	0x10,	0x10,		0x10},	// fixed
								{0x3274,	0x1b,	0x1b,	0x1b,		0x1b},	// fixed
								{0x3275,	0xa0,	0xa0,	0xa0,		0xa0},	// fixed
								{0x3276,	0x02,	0x02,	0x02,		0x02},	// fixed
								{0x32b8,	0x50,	0x50,	0x50,		0x50},	// fixed
								{0x32b9,	0x10,	0x10,	0x10,		0x10},	// fixed
								{0x32ba,	0x00,	0x00,	0x00,		0x00},	// fixed
								{0x32bb,	0x04,	0x04,	0x04,		0x04},	// fixed
								{0x32c8,	0x50,	0x50,	0x50,		0x50},	// fixed
								{0x32c9,	0x10,	0x10,	0x10,		0x10},	// fixed
								{0x32ca,	0x00,	0x00,	0x00,		0x00},	// fixed
								{0x32cb,	0x04,	0x04,	0x04,		0x04},	// fixed
	                                                                	    
								/*ID5*/                             	    
								{0x332c,	0xd3,	0xd3,	0xd3,		0xd3},	// fixed
								{0x332d,	0x10,	0x10,	0x10,		0x10},	// fixed
								{0x332e,	0x0d,	0x0d,	0x0d,		0x0d},	// fixed
								{0x3358,	0x06,	0x06,	0x06,		0x06},	// fixed
								{0x3359,	0xe1,	0xe1,	0xe1,		0xe1},	// fixed
								{0x335a,	0x11,	0x11,	0x11,		0x11},	// fixed
								{0x3360,	0x1e,	0x1e,	0x1e,		0x1e},	// fixed
								{0x3361,	0x61,	0x61,	0x61,		0x61},	// fixed
								{0x3362,	0x10,	0x10,	0x10,		0x10},	// fixed	
								{0x33b0,	0x50,	0x50,	0x50,		0x50},	// fixed
								{0x33b2,	0x1a,	0x1a,	0x1a,		0x1a},	// fixed
								{0x33b3,	0x04,	0x04,	0x04,		0x04},	// fixed
								                                 		    							                                 	
								/*ID6 */                         		    
								{0x3405, 	0x10,	0x00,	0x00, 		0x00},	// REPETITION 			(only MIPI)
								{0x3407, 	0x01,	0x01,	0x01, 		0x03},	// PHYSICAL_LANE_NUM 	(only MIPI)
//								{0x3410, 	0x1A,	0x1A,	0x1A, 		0x1A},	// fixed
//								{0x3414, 	0x0A,	0x0A,	0x0A, 		0x0A},	// OPB_SIZE_V 			(only MIPI)
								{0x3415, 	0x01,	0x00,	0x01, 		0x00},	// DOL : 0x00
								{0x3418, 	0x49,	0x9c,	0x49, 		0x9c},  // Y_OUT_SIZ (normal : 0x449, DOL2P : 0x89c , DOL3P : 0x1155)
								{0x3419, 	0x04,	0x08,	0x04, 		0x08},  // "
								{0x3441, 	0x0C,	0x0A,	0x0C, 		0x0A},  // CSI_DT_FMT (0x0C : 12bit, 0x0A : 10bit)	(only MIPI)
								{0x3442, 	0x0C,	0x0A,	0x0C, 		0x0A},  // CSI_DT_FMT (0x0C : 12bit, 0x0A : 10bit)	(only MIPI)
								{0x3443, 	0x01,	0x01,	0x01, 		0x03},  // CSI_LANE_MODE (4lane)(only MIPI)
								{0x3444, 	0x20,	0x20,	0x20, 		0x20},  // EXTCK_FREQ			(only MIPI)
								{0x3445, 	0x25,	0x25,	0x25, 		0x25},  // EXTCK_FREQ			(only MIPI)
                                         	
								{0x3446, 	0x57,	0x77,	0x77, 		0x77},  // TCLKPOST				(only MIPI)
								{0x3447, 	0x00,	0x00,	0x00, 		0x00},  // TCLKPOST				(only MIPI)
								{0x3448, 	0x37,	0x67,	0x67, 		0x67},  // THSZERO				(only MIPI)
								{0x3449, 	0x00,	0x00,	0x00, 		0x00},  // THSZERO				(only MIPI)
								{0x344A, 	0x1F,	0x47,	0x47, 		0x47},  // THSPREPARE			(only MIPI)
								{0x344B, 	0x00,	0x00,	0x00, 		0x00},  // THSPREPARE			(only MIPI)
								{0x344C, 	0x1F,	0x37,	0x37, 		0x37},  // TCLKTRAIL				(only MIPI)
								{0x344D, 	0x00,	0x00,	0x00, 		0x00},  // TCLKTRAIL				(only MIPI)
								{0x344E, 	0x1F,	0x3F,	0x3F, 		0x3F},  // THSTRAIL				(only MIPI)
								{0x344F, 	0x00,	0x00,	0x00, 		0x00},  // THSTRAIL				(only MIPI)
								{0x3450, 	0x77,	0xFF,	0xFF, 		0xFF},  // TCLKZERO				(only MIPI)
								{0x3451, 	0x00,	0x00,	0x00, 		0x00},  // TCLKZERO				(only MIPI)
								{0x3452, 	0x1F,	0x3F,	0x3F, 		0x3F},  // TCLKPREPARE			(only MIPI)
								{0x3453, 	0x00,	0x00,	0x00, 		0x00},  // TCLKPREPARE			(only MIPI)
								{0x3454, 	0x17,	0x37,	0x37, 		0x37},  // TLPX					(only MIPI)
								{0x3455, 	0x00,	0x00,	0x00, 		0x00},  // TLPX					(only MIPI)
                                         	
								{0x3472, 	0x9C,	0xA0,	0x9C, 		0xA0},  // X_OUT_SIZE			(only MIPI)
								{0x3473, 	0x07,	0x07,	0x07, 		0x07},  // X_OUT_SIZE			(only MIPI)
								{0x347b, 	0x24,	0x23,	0x24, 		0x23},  // MIF_SYNC_TIM0			(only MIPI)
								{0x3480, 	0x49,	0x49,	0x49, 		0x49}   // INCKSEL7
...
// AE
// -----------------------------------------------------------------------------------------
// shutter table value is same with IMX462
...
{	// MIPI RX Configuration
	LibISPtoSENS_Intf(\
					...
					(MIPI),\
					/* 1, 2, 4 LANE */\
					(((aiResMode==MD_2M_FPS_50_60)&&(aiWdrMode==_HDR_LINE))?(_4LANE):(_2LANE)),\
					/* 12BIT or 10BIT */\
					((aiWdrMode==_NORMAL)?(_12BIT):(_10BIT)),\
					/* 3210, 10, 0 */\
					(((aiResMode==MD_2M_FPS_50_60)&&(aiWdrMode==_HDR_LINE))?(_3210):(_10)),\
					/* Word Count */\
...
{	// IMX462
	...
	if(bInitSet){
		...
		if(bChipID==0x2)	{TxStrNoIRQ("Sensor correct matching. ", bChipID, 1);}
...
const s64 glMtxCf[9][6] =		{	// RT, Y 350
								{		 -21798579980546LL, 	   -411925850840LL, 	   -463039996903LL, 		  -950414756LL, 		  -542762621LL, 		  -359109297LL},
								{		  12514899699610LL, 	   -103306602137LL, 		-57108222559LL, 		  -512331656LL, 		  -354900926LL, 		  -264933039LL},
								{		  43960637927387LL, 		 66357981235LL, 		358755192266LL, 		  1002158755LL, 		   303706811LL, 		  1009287158LL},
								{		 -25400717956516LL, 		217732542786LL, 		 84411785933LL, 		   623986508LL, 		   510382132LL, 		   175964819LL},
								{		  74337562302796LL, 	   -316696149532LL, 	   -216575938711LL, 		 -1114117275LL, 		  -769238182LL, 		  -452669092LL},
								{		 -21303143851559LL, 	   -324744565564LL, 		159297962403LL, 		  -109872399LL, 		  -551656280LL, 		   627924703LL},
								{		  -8824021704068LL, 		122243822336LL, 		 -4487141328LL, 		   352782714LL, 		   334356529LL, 			49135240LL},
								{		 -46396115510502LL, 	   -588032416859LL, 	   -551870462350LL, 		 -2153710191LL, 		 -1285927705LL, 		 -1177315319LL},
								{		 285218937790260LL, 	   1352435118765LL, 	   2007833589953LL, 		  6062532197LL, 		  2832361463LL, 		  4428033575LL}
								};
