...
#if(0)
const UINT gnTblToneMapBase[][2] =  {// X    , 		Y 0.01	Y 0.02    
									{0       , /*0x0  */	0x0    	},     // 00
									{4       , /*0x0  */	0x1    	},     // 01
									{8       , /*0x1  */	0x3    	},     // 02
									{16      , /*0x3  */	0x6    	},     // 03
									...
									{6144    , /*0x10C*/	0x137  	},     // 13
									{8192    , /*0x126*/	0x151  	},     // 14
									{12288   , /*0x14C*/	0x175  	},     // 15
									{16384   , /*0x167*/	0x18E  	},     // 16
									{24576   , /*0x18D*/	0x1B2  	},     // 17
									{32768   , /*0x1A8*/	0x1CC  	},     // 18
									{49152   , /*0x1CF*/	0x1F1  	},     // 19
									{65536   , /*0x1EA*/	0x20A  	},     // 20
									{98304   , /*0x211*/	0x22F  	},     // 21
									{131072  , /*0x22C*/	0x249  	},     // 22
									{163840  , /*0x242*/	0x25D  	},     // 23
									{196608  , /*0x253*/	0x26D  	},     // 24
									{229376  , /*0x262*/	0x27B  	},     // 25
									{262144  , /*0x26F*/	0x287  	},     // 26
									{294912  , /*0x27A*/	0x292  	},     // 27
									{327680  , /*0x284*/	0x29B  	},     // 28
									{360448  , /*0x28E*/	0x2A4  	},     // 29
									{393216  , /*0x296*/	0x2AC  	},     // 30
									{458752  , /*0x2A5*/	0x2BA  	},     // 31
									{524288  , /*0x2B1*/	0x2C6  	},     // 32
									{589824  , /*0x2BD*/	0x2D0  	},     // 33
									{655360  , /*0x2C7*/	0x2DA  	},     // 34
									{720896  , /*0x2D0*/	0x2E3  	},     // 35
									{786432  , /*0x2D8*/	0x2EA  	},     // 36
									{851968  , /*0x2E0*/	0x2F2  	},     // 37
									{917504  , /*0x2E7*/	0x2F8  	},     // 38
									{983040  , /*0x2EE*/	0x2FF  	},     // 39
									{1048576 , /*0x2F4*/	0x304  	},     // 40
									{1572864 , /*0x31B*/	0x329  	},     // 41
									{2097152 , /*0x337*/	0x343  	},     // 42
									{2621440 , /*0x34C*/	0x357  	},     // 43
									{3145728 , /*0x35E*/	0x367  	},     // 44
									{3670016 , /*0x36C*/	0x375  	},     // 45
									{4194304 , /*0x379*/	0x381  	},     // 46
									{4718592 , /*0x385*/	0x38C  	},     // 47
									{5242880 , /*0x38F*/	0x396  	},     // 48
									{5767168 , /*0x398*/	0x39E  	},     // 49
									{6291456 , /*0x3A0*/	0x3A6  	},     // 50
									{7340032 , /*0x3AF*/	0x3B4  	},     // 51
									{8388608 , /*0x3BC*/	0x3C0  	},     // 52
									{9437184 , /*0x3C7*/	0x3CB  	},     // 53
									{10485760, /*0x3D1*/	0x3D4  	},     // 54
									{11534336, /*0x3DA*/	0x3DD  	},     // 55
									{12582912, /*0x3E3*/	0x3E5  	},     // 56
									{13631488, /*0x3EB*/	0x3EC  	},     // 57
									{14680064, /*0x3F2*/	0x3F2  	},     // 58
									{15728640, /*0x3F8*/	0x3F9  	},     // 59
									{16777216, /*0x3FF*/	0x3FF  	}      // 60
									};
#else
...
//UINT 	gnAeWdrLSWgt=0x100;				// 180830 LH : WDR weight(L<->S) by external parameter
...
void ISPM LibAeWdr (int aiModeL, int aiModeF, UINT anLef, UINT anSef1, UINT anLgain, UINT anSgain, UINT anWeight)
{	// WDR Tone Map
	// aiModeL  - 0 : Line WDR Off, 1 : 2page
	// aiModeF  - 0 : Frame WDR Off, 1 : WDR
	// aiModeROI- 0 : ROI Off, 1 :  ROI On
	// anLef   - Long
	// anSef1  - Short1
...
//	gnAeWdrLSWgt = MINMAX(gnAeWdrLSWgt, 0, 0x100);	// 180830 LH

	if ((aiModeL==0)&&(aiModeF==0))	{	// WDR all off
	...
	// Normal Mode ToneMap Test
		...
		for (i=0; i<41; i++){
			nTmgValBuf = GetToneMap(gbTblToneMapNor[i][0], gbTblToneMapNor[i][1], PAR01);
		...
		if (aiModeF) {					// LFM+HDR
			...
			if(LFM_SENSOR==0)		PWL_IBITw(0x1);					// Compressed Data 12bit
			else if(LFM_SENSOR==1)	PWL_IBITw(0x3);					// Compressed Data 16bit
			
			TMG_YHLPFw(0);

			SMAX = (anWeight*0xFFF); 											// Short max = 4095 * S gain	TEMP

			iWdrGain   = (((u64)0xFFFFFF*0x100)/(SMAX));						// WDR last gain : 0x100 = x1
			WDR_GAINw(MAX(iWdrGain+giAeLWdrGain, 0x100));						// WDR_GAIN : x1 Default
			
//			DebugDisp(gbAeDebugOn, Hex, "SMAX______", 22, 24, SMAX  	)
//			DebugDisp(gbAeDebugOn, Hex, "WDR_GAIN__", 23, 24, iWdrGain  )
			
		#if(0)
			if(LFM_SENSOR==0){
				//PWL  (Sony - IMX390)
				#if(0)
					PWL_X0w(0x10e);	PWL_Y0w(0x1ff   );
					PWL_X1w(0x4c5);	PWL_Y1w(0x1fff  );
					PWL_X2w(0x5f8);	PWL_Y2w(0x3fff  );
					PWL_X3w(0x730);	PWL_Y3w(0x7fff  );
					PWL_X4w(0x86f);	PWL_Y4w(0xffff  );
					PWL_X5w(0x9ae);	PWL_Y5w(0x1ffff );
					PWL_X6w(0xaf2);	PWL_Y6w(0x3ffff );
					PWL_X7w(0xc37);	PWL_Y7w(0x7ffff );
					PWL_X8w(0xd75);	PWL_Y8w(0xfffff );
					PWL_X9w(0xeba);	PWL_Y9w(0x1fffff);
					PWL_XAw(0xfff);	PWL_YAw(0x3fffff);

				#else
					//SMAX : 1048576
					PWL_X0w(0x39f);	PWL_Y0w(0x1ff   );    
					PWL_X1w(0x75b);	PWL_Y1w(0x1fff  );    
					PWL_X2w(0x852);	PWL_Y2w(0x3fff  );    
					PWL_X3w(0x949);	PWL_Y3w(0x7fff  );    
					PWL_X4w(0xa3b);	PWL_Y4w(0xffff  );    
					PWL_X5w(0xb32);	PWL_Y5w(0x1ffff );    
					PWL_X6w(0xc28);	PWL_Y6w(0x3ffff );    
					PWL_X7w(0xd1f);	PWL_Y7w(0x7ffff );    
					PWL_X8w(0xe11);	PWL_Y8w(0xfffff );    
					PWL_X9w(0xf08);	PWL_Y9w(0x1fffff);    
					PWL_XAw(0xfff);	PWL_YAw(0x3fffff);    
				#endif
			}
			
			else if(LFM_SENSOR==1){
				//PWL (Omnivision - OX03C10
				#if(0)
					PWL_X0w(0x100);		PWL_Y0w(0x100   );    
					PWL_X1w(0x500);		PWL_Y1w(0x700  	);    
					PWL_X2w(0x900);		PWL_Y2w(0x1F00  );    
					PWL_X3w(0x1300);	PWL_Y3w(0x13F00 );    
					PWL_X4w(0x2300);	PWL_Y4w(0x27F00 );    
					PWL_X5w(0x3300);	PWL_Y5w(0x3FF00 );    
					PWL_X6w(0x3F00);	PWL_Y6w(0x7FF00 );    
					PWL_X7w(0x4F00);	PWL_Y7w(0x1FFF00);    
					PWL_X8w(0x7F00);	PWL_Y8w(0x3FFF00);    
					PWL_X9w(0x9F00);	PWL_Y9w(0xFFFF00);    
					PWL_XAw(0xFF00);	PWL_YAw(0xFFFFFF);
				#else
					PWL_X0w(0x100 );	PWL_Y0w(0x100   );    
					PWL_X1w(0x500 );	PWL_Y1w(0x700  	);    
					PWL_X2w(0x900 );	PWL_Y2w(0x1F00 	);    
					PWL_X3w(0x1300);	PWL_Y3w(0x7F00	);    
					PWL_X4w(0x2300);	PWL_Y4w(0x13F00	);    
					PWL_X5w(0x3300);	PWL_Y5w(0x27F00	);    
					PWL_X6w(0x3F00);	PWL_Y6w(0x3FF00 );    
					PWL_X7w(0x4F00);	PWL_Y7w(0x7FF00	);    
					PWL_X8w(0x7F00);	PWL_Y8w(0x1FFF00);    
					PWL_X9w(0x9F00);	PWL_Y9w(0x3FFF00);    
					PWL_XAw(0xFF00);	PWL_YAw(0xFFFF00);
										PWL_YBw(0xFFFFFF);
				#endif
			}
		#endif
			...
			const float fSGainOffset_Max = /*2.0f*/1.0f;
			const float fSGainOffset_Min = /*2.0f*/1.0f;
			...
			f_SGainOffset = LibUtlInterp1F(MINMAX((int)anSef1, (int)gnAeLWdrMinS1, (int)gnAeLWdrMaxS1), (int)gnAeLWdrMinS1, (int)gnAeLWdrMaxS1,\
																								   		 TOFLOAT32(fSGainOffset_Max), TOFLOAT32(fSGainOffset_Min)); // gnAeLWdrMinS1 ~ gnAeLWdrMaxS1
...
//			f_WdrGainS1 = _fdiv(_fmul(f_anLef,f_Sgain),f_anSef1); 	// f_WdrGainS1 = (Long*0x40 / Short Shutter) * (Long / Short Gain)
...
//			f_Smax = _fdiv(_fmul(f_anLef,f_Smax),f_anSef1); // Short max = 4095 * S gain
	...
	#else
			const WORD wSMAXIIRCF = 0x40;
			UINT Sgain = 0x40, Smax_K = 0xFFF;	// Gain Ratio
			float32 fAgc = 0x1, f_Sgain, f_WdrGainS1, f_SMAX; // Gain, SMAX
			float32 f_anLef = 0, f_anSef1 = 0;	// Shutter

			nSMAXIIR = SMAX;
			
			// int to float32
			f_anLef = _itof((int)anLef); // Long
			f_anSef1 = _itof((int)anSef1); // Short1
			f_Sgain = _itof((int)Sgain); // S_Gain
			f_SMAX = _itof((int)Smax_K); // Smax
			
			fAgc = _fdiv(_itof((int)anLgain),_itof((int)anSgain));				// Long / Short Gain Ratio

//			const float fSGainOffset_Max = 1.422f;
			const float fSGainOffset_Max = 2.0f;
			const float fSGainOffset_Min = 2.0f;

			float fSGainOffset = 1.0f;

			// (Sef1_Min -> Offset_Max) ~ (Sef1_MAX -> Offset_Min)
			fSGainOffset = LibUtlInterp1F (MINMAX(anSef1,1,17), 1/*gnAeLWdrMinS1*/, 17/*gnAeLWdrMaxS1*/, fSGainOffset_Max, fSGainOffset_Min); // gnAeLWdrMinS1 ~ gnAeLWdrMaxS1

			// WDR S1 Gain
			f_WdrGainS1 = _fmul((_fdiv(f_anLef,f_anSef1)),(fAgc)); // f_WdrGainS1 = (Long / Short Shutter) * (Long / Short Gain)
			f_WdrGainS1 = _fmul(f_WdrGainS1, f_Sgain);				// f_WdrGainS1 = f_WdrGainS1 * 0x40
			iWdrGainS1 = _ftoi(f_WdrGainS1);
			iWdrGainS1 = (iWdrGainS1 * fSGainOffset); // for Offset Test

			// SMAX
			f_SMAX = _fmul((_fmul((_fdiv(f_anLef,f_anSef1)),(fAgc))), (f_SMAX)); // Short max = 4095 * S gain
			SMAX = _ftoi(f_SMAX);
			SMAX = (SMAX * fSGainOffset); // for Offset Test
...
//            bYHLPF_SC = LibUtlInterp1D (MINMAX(giCurAgc, 0x0, 0x30), 0x0, 0x30, 0x30, 0x0);
		...
		if (aiModeL || aiModeF) {					// Tone map for WDR
...
//									{SetIsp(TMG_BASE+i,(nTblToneMapCurY[i*2]<<16));}
