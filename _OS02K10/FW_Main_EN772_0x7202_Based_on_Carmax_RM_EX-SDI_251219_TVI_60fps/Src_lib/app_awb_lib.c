...
//	float 	fWdrClipB;
	...
	hfloat 	fWdrClipR_S;		// 171207 LH : HW floating
	hfloat 	fWdrClipG_S;
	hfloat 	fWdrClipB_S;
...
//		WDR_SCLIPBw((int)fWdrClipB);
	...
	// 171207 LH : HW floating -------------------------------------------------
		...
		fWdrClipR_S = _fdiv(i_h(RR_INV+RG_INV+RB_INV), i_h(DET));
		fWdrClipG_S = _fdiv(i_h(GR_INV+GG_INV+GB_INV), i_h(DET));
		fWdrClipB_S = _fdiv(i_h(BR_INV+BG_INV+BB_INV), i_h(DET));
		...
		if(gbModel_Sens==SENS_PWL_OS02K10){
			fWdrClipR = aiNight ? i_h(0xffff) : _fmul(_fdiv(fWdrClipR,fWdrClipMax),i_h(65535));
			fWdrClipG = aiNight ? i_h(0xffff) : _fmul(_fdiv(fWdrClipG,fWdrClipMax),i_h(65535));
			fWdrClipB = aiNight ? i_h(0xffff) : _fmul(_fdiv(fWdrClipB,fWdrClipMax),i_h(65535));	
		}
		else{
		...
		fWdrClipR_S = aiNight ? i_h(0xfff) : _fmul(_fdiv(fWdrClipR_S,fWdrClipMax),i_h(4095));
		fWdrClipG_S = aiNight ? i_h(0xfff) : _fmul(_fdiv(fWdrClipG_S,fWdrClipMax),i_h(4095));
		fWdrClipB_S = aiNight ? i_h(0xfff) : _fmul(_fdiv(fWdrClipB_S,fWdrClipMax),i_h(4095));
	#endif
	
		WDR_LCLIPRw(h_i(fWdrClipR));
		WDR_LCLIPGw(h_i(fWdrClipG));
		WDR_LCLIPBw(h_i(fWdrClipB));
		WDR_SCLIPRw(h_i(/*fWdrClipR*/fWdrClipR_S));
		WDR_SCLIPGw(h_i(/*fWdrClipG*/fWdrClipG_S));
		WDR_SCLIPBw(h_i(/*fWdrClipB*/fWdrClipB_S));
	// ------------------------------------------------------------------------
